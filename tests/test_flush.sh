...
echo "set output \"flush.eps\"" > ../gnuplot_instructions.txt
echo "set terminal postscript eps enhanced;" >> ../gnuplot_instructions.txt
echo "plot \"data.txt\" with lines;" >> ../gnuplot_instructions.txt

echo "\subsection{flush()}" >> ../tex/report.tex
echo "Function used for starting the execution of all" >> ../tex/report.tex
echo " queued drawing functions and updating the window" >> ../tex/report.tex
echo " with the contents in the buffer (we use double-buffering)." >> ../tex/report.tex
gnuplot ../gnuplot_instructions.txt
mv flush.eps ../tex
echo "" >> ../tex/report.tex
echo "\includegraphics{tests/tex/flush.eps}" >> ../tex/report.tex
echo "" >> ../tex/report.tex
