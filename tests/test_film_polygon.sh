...
echo "set output \"film_polygon.eps\"" > ../gnuplot_instructions.txt
...
echo "\subsection{film\_polygon(a,b)}" >> ../tex/report.tex
echo "Function used for drawing a polygon in the" >> ../tex/report.tex
echo "screen, given it's coordinates. " >> ../tex/report.tex
gnuplot ../gnuplot_instructions.txt
mv film_polygon.eps ../tex
VALUE=$(tail -n 1 data.txt | cut -d " " -f 2)
VALUE=$(echo "scale=2; ${VALUE}/10000" | bc -l)
echo "" >> ../tex/report.tex
echo "\includegraphics{tests/tex/film_polygon.eps}" >> ../tex/report.tex
echo "" >> ../tex/report.tex
